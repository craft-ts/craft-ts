...
For the standard development observability surface, start these from the
...
The app installs the same development-only Craft log forwarding and browser
MCP bridge as projects generated by `craft create`. Use `Console.*` for logs
that should be searchable through the log MCP server.
