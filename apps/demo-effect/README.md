...
The `/sync-members` example also exposes the synchronous cart-total Effect
program as a callable Craft method through `methodEffect`, alongside
`computedEffect` and `queryEffect`.
