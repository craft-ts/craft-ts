# Lazy Services (`craftLazy`)

`craftLazy(load)` lazily imports a module **on demand** from inside an async craft driver — an
[`asyncProcess`](../primitives/async-process.md) loader, or a route guard/resolver — reusing the
exact same retry + cache-busting engine as route lazy loading
([`loadComponent` / `loadChildren`](./route-load-errors.md)).
...
- `craftLazy(...)` is a [`craftGen`](./craft-gen.md) program: `yield*`-composable and
  [`.pipe(...)`](./program-operators.md)-able.
...
[`craftUntilSettled`](./guards.md) and calls the loaded function. Wrapping both in a
[`craftService`](../store/craft-service.md) exposes a clean API:
...
[exception handlers](./exception-handling.md), exactly like any other guard/resolver exception:
...
See [Program Operators](./program-operators.md) for `catchTag` / `catchTag.exhaustive`.
...
(useful in tests). This is the very same engine as [route load retry](./route-load-errors.md), so a
