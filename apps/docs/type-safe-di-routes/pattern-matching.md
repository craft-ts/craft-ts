...
[`catchTag` / `catchTag.exhaustive`](./program-operators.md) pair: one call for a single case, a
...
- **Pure and synchronous.** Unlike [`catchTag`](./program-operators.md), `craftMatch` is not a craft
