    ...
    <div class="content-wrapper">
      <div class="card">
        <h2 class="card-title">
          ...
          <app-status
            [status]="
              usersQuery.currentPageStatus()
            "
          />
        </h2>

        <div>
        {{ usersQuery.isPlaceHolderData() ? 'Showing placeholder data' : 'Showing real data' }}
</div>

        TODO Attention: when changing page and going on a previous page with loaded data, it will fetch again
        TODO Le insertLocalStoragePersister ne fonctionne pas avec le queryparam ??
        TODO remove set and other method from queryParam value
        ...
        <div class="table-container">
          <table class="table">
            ...
            <tbody>
              ...
              @for(user of
              usersQuery.currentPageData(); track
              user.id) {
              <tr>
                <td>{{ user.id }}</td>
        ...
        <div class="pagination">
          ...
          <button class="btn" (click)="pagination.nextPage()">
            Next
          </button>
        </div>
      </div>
...
export default class ListWithPagination {
  ...
  protected updatePageSize(event: Event) {
    ...
    this.pagination.set({ page: 1, pageSize: value });
  }
}
