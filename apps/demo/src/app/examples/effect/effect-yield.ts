  ...
  ({ run, traceEntries, userQuery, userExceptionLoader, userIsLoading }) => {
    return div([
      ...
      div({ class: 'effect-gap' }, [
        strong('The gap this prototype found. '),
        'Everything above is runtime behaviour, and it works. The types know none of it: ',
        span({ class: 'mono' }, 'E'),
        ' never reaches the exception channel, so ',
        span({ class: 'mono' }, 'userQuery.exception()'),
        ' is statically typed ',
        span({ class: 'mono' }, 'undefined'),
        ' — an exception is not even representable — while one is provably sitting in it. That is why the ',
        span({ class: 'mono' }, 'matchBlock'),
        ' above needs a cast, and why "your Effect errors, checked at compile time" is not true yet. Closing it is plan task 2.4 / 2.5.',
      ]),
    ]);
  },
);
