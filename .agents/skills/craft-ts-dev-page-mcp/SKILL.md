...
Operate the tab the developer already has open. Do **not** open Playwright, a second browser, or Chrome DevTools for this. The tool is `page` on `@craft-ts/function-registry-mcp` (`npm run registry:mcp`). It does not exist on `@craft-ts/mcp`.

Cursor loads skills from `.cursor/skills/`. This file is mirrored at `.cursor/skills/craft-ts-dev-page-mcp/SKILL.md`.
...
See [guide/ai/dev-page](../../../apps/docs/guide/ai/dev-page.md) and the sibling skill `craft-ts-runtime-change-web-mcp` for primitive mutations.
