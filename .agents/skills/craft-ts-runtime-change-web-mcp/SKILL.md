...
Do not use `registry.call` to emulate an override. If the user asks to change what a button does on future clicks, use `registry.override`. To click or fill the open tab like a user, use the `page` tool (skill `craft-ts-dev-page-mcp`), not `registry.call` on a button.
