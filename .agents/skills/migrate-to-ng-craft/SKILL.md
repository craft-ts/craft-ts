---
name: migrate-to-ng-craft
description: Migrate an Angular application to @craft-ng/core with the craft-migrate codemod, then resolve its manual diagnostics and verify the result. Use when the user asks to migrate an Angular project, services, signals, Signal Forms, dependency injection, HTTP resources, or routes to Craft NG; asks to run craft-migrate; or needs help finishing work left by the migration scripts.
---
...
1. Inspect the repository before modifying it:
   ...
   - Read the installed `@craft-ng/core` version and avoid generating APIs that
     version does not expose.

2. Install or build `@craft-ng/dev-tools` only when needed.
