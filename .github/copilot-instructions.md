...
- For a single file without `GenDeps_*`, prefer the ESLint Quick Fix from `craft-ng/brand-angular-gen-deps-required`.
- For a single file with `GenDeps_*`, prefer the ESLint Quick Fix from `craft-ng/brand-angular-deps-match` or run `eslint --fix path/to/file.ts`.
