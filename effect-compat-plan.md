...
| discriminant passé en paramètre du match | vrai des **deux** : `matchNode.exhaustive(source, key, handlers)` côté template (`libs/component/src/lib/match-node.ts:73`) et `craftMatch.exhaustive(value, key, handlers)` côté valeur (`craft-match.ts:65-81`) |
...
`matchNode.exhaustive(source, key, handlers)` prend déjà le discriminant en
