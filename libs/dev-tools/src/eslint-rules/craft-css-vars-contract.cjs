...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      missing:
        'External styles use {{name}} but ComponentMeta.cssVars does not declare it.',
      absent:
        'External styles use CSS variables but ComponentMeta.cssVars is missing.',
      unused:
        'ComponentMeta.cssVars declares {{name}}, but the stylesheet never declares or consumes it.',
    },
  },
  create(context) {
      ...
      CallExpression(node) {
        ...
        if (info.cssVars) {
          facts.used.forEach((name) => {
            if (!info.contract.has(name))
              context.report({
                node: info.cssVars.value,
                messageId: 'missing',
                data: { name },
              });
          });
          info.contract.forEach((name) => {
            if (!facts.used.has(name) && !facts.declared.has(name))
              context.report({
                node: info.cssVars.value,
                messageId: 'unused',
                data: { name },
