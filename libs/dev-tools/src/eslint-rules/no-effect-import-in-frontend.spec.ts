...
describe('no-effect-import-in-frontend', () => {
  it('rejects direct EffectTS and Craft Effect imports', async () => {
    await expect(
      lint(
        `import { Effect } from 'effect'; import { queryEffect } from '@craft-ts/effect'; import '@effect/platform';`,
      ),
  ...
  it('rejects re-exports and dynamic imports', async () => {
    await expect(
      lint(
        `export * from '@craft-ts/i18n-effect'; void import('effect/Schema');`,
      ),
  ...
  it('accepts plain Craft and non-Effect imports', async () => {
    await expect(
      lint(
        `import { query } from '@craft-ts/core'; import { format } from './format';`,
      ),
    ).resolves.toEqual([]);
  });
});
