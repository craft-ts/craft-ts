 ...
 */
function* waitForReplayReady(view: FrameView) {
...
const initialDevtoolView = (): DevtoolView => 'review';
const initialRetirementReason = (): RetirementReason => 'superseded';
const initialKindFilter = (): KindFilter => 'all';
const initialStateFilter = (): StateFilter => 'all';
const initialDirectionFilter = (): DirectionFilter => 'all';
const initialZoom = (): ZoomMode => 'fit';
const initialEvidenceView = (): EvidenceView => 'auto';
const initialMentions = (): readonly Mention[] => [];
const initialSelection = (): readonly string[] => [];

const isRetirementReason = (value: string): value is RetirementReason =>
...
const isStateFilter = (value: string): value is StateFilter =>
  ...
  value === 'review' ||
  value === 'removed';
const isDirectionFilter = (value: string): value is DirectionFilter =>
  value === 'all' || value === 'render' || value === 'command';
const isZoomMode = (value: string): value is ZoomMode =>
...
const reviewLogic = craftGen(function* () {
  ...
  const selectedIndex = yield* state('selectedIndex', 0, ({ set }) => ({
    selectFromNavigation: on$(navigation$, (index) => set(index)),
  }));
  ...
  const kindFilter = yield* state(
    ...
    initialKindFilter(),
    ({ set }) => ({
      chooseFromInput: (value: KindFilter) => set(value),
  ...
  const stateFilter = yield* state(
    ...
    initialStateFilter(),
    ({ set }) => ({
      chooseFromInput: (value: StateFilter) => set(value),
      clearFromFilterEvent: on$(clearFilters$, () => set('all')),
    }),
  );
  const directionFilter = yield* state(
    ...
    ({ set }) => ({
      chooseFromInput: (value: DirectionFilter) => set(value),
      clearFromFilterEvent: on$(clearFilters$, () => set('all')),
    }),
  );
  ...
  const visualAssets = craftComputed('visualAssets', function* () {
    ...
    return ((yield* review.value())?.visualAssets ?? []).filter((asset) => {
      ...
      return (
        ...
        (!text || `${asset.evidence} ${scenarios}`.toLowerCase().includes(text))
      );
    });
  });
  ...
  const movePrevious = craftMethod('movePrevious', function* () {
    ...
    clearReason();
    navigation$.emit(Math.max(0, index - 1));
  });
  const moveNext = craftMethod('moveNext', function* () {
    const list = yield* cards();
    const index = yield* selectedIndex();
    clearReason();
    navigation$.emit(Math.min(Math.max(0, list.length - 1), index + 1));
  });
  ...
  const trackSelection = craftMethod(
    ...
    function* (paths: readonly string[]) {
      ...
      if (previous && !/\s$/.test(previous)) {
        chip.parentNode?.insertBefore(reviewDocument.createTextNode(' '), chip);
      }
...
type ReviewContext =
  ReturnType<typeof reviewLogic> extends Generator<
    unknown,
    infer Result,
    unknown
  >
...
const reviewTemplate = craftTemplate(
  ({
    review,
    decision,
    cards,
    ...
    inspectFrame,
    toggleChrome,
  }: YieldableTemplateContext<ReviewContext>) =>
    div({ class: 'app-shell' }, [
      ifNode(reviewFailed, () =>
            ...
            div({ class: 'preferences' }, [
              ...
              select(
                'ReviewLocale',
                {
                  ...
                  *change(event: Event) {
                    ...
                    if (isLocale(value)) yield* chooseLocale(value);
                  },
                },
                [
              ...
              select(
                'ReviewTheme',
                {
                  ...
                  *change(event: Event) {
                    const value = eventValue(event);
                    if (isThemeChoice(value)) yield* chooseTheme(value);
                  },
                },
                [
              ...
              button(
                ...
                [
                  ...
                  span({ class: 'view-tab-count' }, function* () {
                    return String((yield* visualTests()).length);
                  }),
                ],
              ),
              button(
                ...
                [
                  ...
                  span({ class: 'view-tab-count' }, function* () {
                    return String((yield* templateObligations()).length);
                  }),
                ],
              ),
              button(
                ...
                [
                  ...
                  span({ class: 'view-tab-count' }, function* () {
                    return String((yield* cards()).length);
                  }),
                ],
              ),
            ],
              ...
              div({ class: 'filters-heading' }, [
                ...
                div({ class: 'filter-heading-actions' }, [
                  small(
                    ...
                    function* () {
                      return (yield* t()).activeFilters(
                        yield* activeFilterCount(),
                      );
                    },
                  ),
                  button(
                  ...
                  'KindFilter',
                  {
                    ...
                    *change(event: Event) {
                      ...
                      if (isKindFilter(value)) {
                        yield* kindFilter.chooseFromInput(value);
                      }
                    },
                  },
                  [
                  ...
                  'StateFilter',
                  {
                    ...
                    *change(event: Event) {
                      ...
                      if (isStateFilter(value)) {
                        yield* stateFilter.chooseFromInput(value);
                      }
                    },
                  },
                  [
              ...
              div({ class: 'filter-field' }, [
                ...
                select(
                  'DirectionFilter',
                  {
                    ...
                    *change(event: Event) {
                      const value = eventValue(event);
                      if (isDirectionFilter(value)) {
                        yield* directionFilter.chooseFromInput(value);
                      }
                    },
                  },
                  [
                      ...
                      [
                        ...
                        select(
                          'RetirementReason',
                          {
                            ...
                            *change(event: Event) {
                              const value = eventValue(event);
                              if (isRetirementReason(value)) {
                                yield* chooseRetirementReason(value);
                              }
                            },
                          },
                          [
