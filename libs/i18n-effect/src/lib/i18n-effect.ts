...
export class I18nEffectService extends Context.Service<
  I18nEffectService,
  I18nEffectShape
...
>(runtime: I18nRuntime<Locales>): Layer.Layer<I18nEffectService> {
 ...
 */
const translateEffectUntraced = Effect.fnUntraced(function* (
  key: string,
  params?: Record<string, unknown>,
) {
  const service = yield* I18nEffectService;
  const translate = service.runtime.translate as unknown as (
    key: string,
    params?: Record<string, unknown>,
  ) => string;
...
>(
  ...
  return translateEffectUntraced(
    key,
