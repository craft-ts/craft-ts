...
import { smartInject } from './smart-inject';
...
describe('smartInject', () => {
  ...
  it('1- Should only expose the declared properties', () => {
    TestBed.runInInjectionContext(() => {
      const serviceA = TestBed.inject(ServiceA);
      serviceA.reset();

      const smartInjectServiceA = smartInject(ServiceA, (serviceA) => ({
  ...
  it('3- Should expose previous insertions in later callbacks', () => {
    TestBed.runInInjectionContext(() => {
      const serviceA = TestBed.inject(ServiceA);
      serviceA.reset();

      const smartInjectServiceA = smartInject(
        ServiceA,
        ({ value }) => ({
          value,
          _triplePlusOne: computed(() => value() * 3 + 1),
        }),
        ({ value, _triplePlusOne }) => ({
          summary: computed(() => `${value()}-${_triplePlusOne()}`),
        }),
      );
      ...
      expect(smartInjectServiceA.summary()).toBe('0-1');

      smartInjectServiceA.value.set(2);

      expect(smartInjectServiceA.summary()).toBe('2-7');
      expect('_triplePlusOne' in smartInjectServiceA).toBe(false);
    });
  });

  it('4- Should bind all service entries from the second parameter', async () => {
    @Injectable({
      providedIn: 'root',
    })
    class MyService {
  ...
  it('5- Should support mandatory entry bindings with public methods and hidden subscriptions', async () => {
