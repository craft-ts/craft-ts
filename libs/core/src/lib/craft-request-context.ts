...
import { craftHandshakeName, type CraftHandshakeName, type CraftHandshakeSchema } from './craft-handshake';
...
const contextsByHandshake = new Map<string, CraftRequestContext<string, CraftSchema>>();

export function craftRequestContext<Name extends string, Schema extends CraftSchema>(
...
export function craftRequestContext(
  handshake: CraftHandshakeName<string> | CraftHandshakeSchema<string, CraftSchema>,
  schema?: CraftSchema,
): CraftRequestContext<string, CraftSchema> {
  const name = craftHandshakeName(handshake);
  if (!name) throw new Error('craftRequestContext expects a named craftHandshake.');
  const shape = schema ?? (handshake as CraftSchema);
  if (!shape['~standard']) {
    throw new Error(`Request context "${name}" requires a schema.`);
  }
  const service = Context.Service<unknown, unknown>(`craft/request-context/${name}`) as CraftRequestContext<string, CraftSchema>;
...
export function isCraftRequestContext(value: unknown): value is CraftRequestContext<string, CraftSchema> {
  return typeof value === 'object' && value !== null &&
    (value as { [CRAFT_REQUEST_CONTEXT]?: unknown })[CRAFT_REQUEST_CONTEXT] === true;
}

export function requestContextForHandshake(value: unknown): CraftRequestContext<string, CraftSchema> | undefined {
...
export function provideCraftRequestContexts<Value>(
  ...
  for (const schema of schemas) {
    ...
    if (context) {
      provided = Effect.provideService(provided, context, value) as Effect.Effect<Value, unknown, unknown>;
    }
  }
  return provided;
}
